...
export default function GitHubCopilot() {
  ...
  const connectViaPat = async () => {
    ...
    setMessage("");
    try {
      const r = await fetch(`${BASE}/github-copilot/connect-pat`, {
        method: "POST",
      });
      ...
      if (d.success) {
        ...
        setStep("error");
        setMessage(d.error || "Error al conectar");
      }
    } catch {
      setStep("error");
  ...
  const startDeviceFlow = async () => {
    ...
    setMessage("");
    try {
      const r = await fetch(`${BASE}/github-copilot/device-flow`, {
        method: "POST",
      });
      const d = await r.json();
      if (d.error) {
      ...
      const pollAsync = async () => {
        try {
          const pr = await fetch(`${BASE}/github-copilot/poll/${d.flowId}`, {
            method: "POST",
          });
          ...
          if (pd.status === "success") {
            stopPolling();
            setStep("success");
            setMessage(pd.message || "Conectado");
          } else if (pd.status === "expired") {
            stopPolling();
            setStep("error");
            setMessage("El código expiró. Inténtalo de nuevo.");
          } else if (pd.status === "error") {
            stopPolling();
            setStep("error");
            setMessage(pd.error || "Error de autorización");
          } else if (pd.interval) {
            clearInterval(pollRef.current!);
            interval = pd.interval * 1000;
      ...
      setStep("error");
      setMessage("Error al conectar con GitHub");
    }
  };
    ...
    <div className="space-y-6 max-w-lg">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-0.5">
            Usa GPT-4o, Claude, Gemini y más vía tu suscripción Copilot.
          </p>
        </div>
      </div>
      ...
      {connected && step === "idle" && (
        <div className="flex items-center gap-3 p-4 rounded-xl bg-green-500/10 border border-green-500/20 text-sm text-green-400">
          ...
          <span>
            Copilot conectado — usando <strong>{connectedModel}</strong>
          </span>
          <button
            onClick={reset}
      ...
      <AnimatePresence mode="wait">
          ...
          >
                ...
                <div className="flex items-start gap-3 mb-4">
                  ...
                  <div>
                    <h3 className="font-semibold text-foreground text-sm">
                      Conexión rápida con tu token de GitHub
                    </h3>
                    <p className="text-xs text-muted-foreground mt-0.5">
                      Usa el Personal Access Token que ya tienes configurado.
                      Instantáneo.
                    </p>
                  </div>
                </div>
                <button
                  onClick={connectViaPat}
                  disabled={loading}
                  className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl bg-primary text-primary-foreground text-sm font-semibold hover:opacity-90 transition-opacity disabled:opacity-60"
                >
                  {loading ? (
                  ...
                  {loading
                    ? "Conectando..."
                    : "Conectar con mi token de GitHub"}
                </button>
              </div>
            )}
            ...
            <div className="bg-card border border-border rounded-2xl p-5">
              <div className="flex items-start gap-3 mb-4">
                ...
                <div>
                  <h3 className="font-semibold text-foreground text-sm">
                    Autorización por código {hasPat ? "(alternativo)" : ""}
                  </h3>
                  <p className="text-xs text-muted-foreground mt-0.5">
                    Genera un código, visita GitHub y autoriza manualmente.
                  </p>
                </div>
              </div>
              <div className="bg-muted/30 rounded-xl p-3 text-xs text-muted-foreground space-y-1 mb-4">
                ...
                <div className="flex flex-wrap gap-1.5 mt-1">
                  {COPILOT_MODELS.map((m) => (
                    <span
                      key={m}
                      className="bg-background border border-border px-2 py-0.5 rounded-full text-foreground"
                    >
                      {m}
                    </span>
                  ))}
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl bg-[#24292e] text-white text-sm font-semibold hover:bg-[#1a1f24] transition-colors border border-white/10"
              >
                <Github size={15} /> Iniciar con código de autorización
              </button>
            </div>
            <p className="text-xs text-muted-foreground text-center">
              Requiere suscripción activa a GitHub Copilot
            </p>
          </motion.div>
        )}
          ...
          >
            ...
            <div className="p-6 space-y-5">
              <p className="text-sm text-muted-foreground">
                ...
                e introduce el código para conectar tu cuenta.
              </p>
              <div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    onClick={copyCode}
                    className="p-3 rounded-xl bg-secondary border border-border hover:bg-muted transition-colors shrink-0"
                  >
                    {copied ? (
              ...
              <div className="flex items-center gap-3 py-3 border-t border-border/50">
                ...
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="ml-auto flex items-center gap-1.5 text-xs text-primary hover:underline shrink-0"
                >
        ...
        {step === "success" && (
          <motion.div
            ...
            className="bg-card border border-green-500/30 rounded-2xl p-8 text-center space-y-4"
          >
            ...
            <button
              onClick={reset}
              className="flex items-center gap-2 px-4 py-2 rounded-xl bg-secondary text-muted-foreground text-sm hover:text-foreground transition-colors mx-auto"
            >
        ...
        {step === "error" && (
          <motion.div
            ...
            initial={{ opacity: 0, scale: 0.95 }}
            animate={{ opacity: 1, scale: 1 }}
            className="bg-card border border-red-500/30 rounded-2xl p-6 space-y-4"
          >
