import { db, productsTable } from "./lib/db/dist/index.js";
...
async function checkProducts() {
  ...
  try {
    ...
    if (allProducts.length === 0) {
      ...
      console.log("3. Usa la API para importar\n");
      process.exit(0);
    }
    ...
    const pianoProducts = allProducts.filter(p => 
      p.name.toLowerCase().includes("piano") && p.isActive
    );
